...
from .pixel_difference_convolutionPDC import *
...
class DiffusionDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # Build Dynamic Head.
        #在这里加了中心卷积的计算
        keys = [i for i in self.backbone.output_shape().keys()]

        # keys: ['p2', 'p3', 'p4', 'p5', 'p6']
        channels = self.backbone.output_shape()[keys[-1]].channels
        # self.diff_conv = Conv2d(CPDC, channels, channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=False)


        # channels_p2 = self.backbone.output_shape()[keys[0]].channels
        # channels_p3 = self.backbone.output_shape()[keys[1]].channels
        # channels_p4 = self.backbone.output_shape()[keys[2]].channels
        channels_p5 = self.backbone.output_shape()[keys[3]].channels
        # channels_p6 = self.backbone.output_shape()[keys[4]].channels
        # self.diff_conv2 = Conv2d(CPDC, channels_p2, channels_p2, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,bias=False)
        # self.diff_conv3 = Conv2d(CPDC, channels_p3, channels_p3, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,bias=False)
        # self.diff_conv4 = Conv2d(CPDC, channels_p4, channels_p4, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,bias=False)
        self.diff_conv5 = Conv2d(CPDC, channels_p5, channels_p5, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,bias=False)
        # self.diff_conv6 = Conv2d(CPDC, channels_p6, channels_p6, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,bias=False)
    ...
    def ddim_sample(self, batched_inputs, backbone_feats, images_whwh, images, clip_denoised=True, do_postprocess=True):
        ...
        for time, time_next in time_pairs:
            ...
            if self.box_renewal:  # filter 冗余无用的框删除，添加随机的框
                score_per_image, box_per_image = outputs_class[-1][0], outputs_coord[-1][0]
                threshold = 0.5
                score_per_image = torch.sigmoid(score_per_image)
                #选取置信度最大的
                value, _ = torch.max(score_per_image, -1, keepdim=False)
                # 和阈值比较，得到布尔列表，求和，一共有多少超过阈值的
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        # Feature Extraction.
        # ROI HEADS : in features
        src = self.backbone(images.tensor)
        features = list()
        feature_p2 = src[self.in_features[0]]
        features.append(feature_p2)
        feature_p3 = src[self.in_features[1]]
        features.append(feature_p3)
        feature_p4 = src[self.in_features[2]]
        # features.append(self.diff_conv4(feature_p4))
        features.append(feature_p4)
        feature_p5 = src[self.in_features[3]]
        # features.append(self.diff_conv4(feature_p4))
        features.append(self.diff_conv5(feature_p5) + feature_p5)
        # for f in self.in_features[4:]:
        #     feature = src[f]
        #     features.append(feature)

        # feature_1 = src[self.in_features[-1]]
        # features.append(self.diff_conv(feature_1) + feature_1)
        # feature_p2 = src[self.in_features[0]]
        # features.append(feature_p2)
        #
        # feature_p3 = src[self.in_features[1]]
        # features.append(self.diff_conv3(feature_p3) + feature_p3)
        #
        # feature_p4 = src[self.in_features[2]]
        # features.append(self.diff_conv4(feature_p4) + feature_p4)
        #
        # feature_p5 = src[self.in_features[3]]
        # features.append(self.diff_conv5(feature_p5) + feature_p5)

        # feature_p6 = src[self.in_features[4]]
        # features.append(self.diff_conv6(feature_p6))
    ...
    def prepare_diffusion_repeat(self, gt_boxes):
        ...
        #进行scale操作，增加框的面积，增加信息量
        gt_boxes = (gt_boxes * 2. - 1.) * self.scale
        x_start = torch(gt_boxes, repeat_tensor, dim=0)
